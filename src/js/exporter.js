...
function toggleElement(id){var el=document.getElementById(id);if(el){el.style.display=el.style.display==='none'?'':'none';}}
  ...
  async function exportProjectZip(project) {
    ...
    const zip = new JSZip();
    const ext = codeManifest.extension || '.html';
    const outputFolder = (codeManifest.outputFolder || '').replace(/^\/+|\/+$/g, '');

    const pages = project.pages || [{ id: 'p', name: project.name || 'index', tree: project.tree || [] }];
...
function toggleElement(id){var el=document.getElementById(id);if(el){el.style.display=el.style.display==='none'?'':'none';}}
      ...
      const fileContent = codeTemplate
        .replace('{{HEAD}}', headInjection)
        .replace('{{CONTENT}}', body)
        .replace('{{TITLE}}', page.name || project.name || 'Untitled');

      const fileName = (page.name || 'page') + ext;
      const path = outputFolder ? `${outputFolder}/${fileName}` : fileName;
      zip.file(path, fileContent);
    });

    const readme = generateReadme(project, codeManifest, cssCdn);
  ...
  function generateReadme(project, codeManifest, cssCdn) {
    return `# ${project.name || 'Untitled'}

Generated by PullStack — drag-and-drop page builder.

## Target framework

- Code: **${codeManifest.name || project.codeFramework}** (${codeManifest.extension || '.html'})
- CSS: **${project.cssFramework}**

## How to use

${codeManifest.outputFolder
  ? `Place the file(s) inside your project's \`${codeManifest.outputFolder}\` directory.`
  : 'Open the HTML file in any browser.'}

The generated file already includes a CDN reference for the chosen CSS framework:

\`\`\`html
${cssCdn.trim()}
\`\`\`

For production, replace the CDN with a locally-built version of the framework
(\`npm install\` or your framework's normal install path).

## Pages in this project

${(project.pages || []).map(p => `- ${p.name}${codeManifest.extension || '.html'}`).join('\n') || '- (single page)'}

## Re-edit

Open the matching \`.pullstack.json\` in PullStack to keep iterating on this design.
`;
  }
